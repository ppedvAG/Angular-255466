...
export class TaskRemoteService {
  ...
  loadTasks(): Promise<TaskItem[]> {
    ...
    const response$ = this.http.get<TaskItem[]>(`${API_URL}`).pipe(
  ...
  async addTask(task: Omit<TaskItem, 'id' | 'completed'>) {
    ...
    return lastValueFrom(this.http.post(`${API_URL}`, newTask).pipe(this.reload));
  }

  async updateTask(id: string, task: Partial<TaskItem>) {
    const newTask = { ...this.getTask(id), ...task };
    return lastValueFrom(this.http.put(`${API_URL}/${id}`, newTask).pipe(this.reload));
  }

  async removeTask(id: string) {
    return lastValueFrom(this.http.delete(`${API_URL}/${id}`).pipe(this.reload));
  }
}
